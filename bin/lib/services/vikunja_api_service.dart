...
import 'package:vikunja_flutter_api/vikunja_api/lib/api.dart'
    as vikunja; // Import Vikunja API client
...
class VikunjaApiService implements TaskApiService {
  ...
  void _initializeClient(AuthStrategy? strategy, String baseUrl) {
    try {
      ...
      final auth = strategy?.createVikunjaAuth(); // Assumes createVikunjaAuth exists
      ...
      stderr.writeln(
        '[VikunjaApiService] Error initializing client: $e',
      );
  ...
  Future<List<TaskItem>> listTasks({
    ...
    try {
      ...
      final String serverId = targetServerOverride?.id ?? "vikunja_default"; // Example server ID
      final taskItems = tasks?.map((task) => _toTaskItem(task, serverId)).toList() ?? [];
  ...
  Future<TaskItem> getTask(
    ...
    try {
      ...
      final String serverId = targetServerOverride?.id ?? "vikunja_default";
      final taskItem = _toTaskItem(task, serverId);
  ...
  Future<TaskItem> createTask(
    ...
    try {
      ...
      final createdVikunjaTask = await _tasksApi.projectsIdTasksPut(targetProjectId, request);
      ...
      if (verboseLogging) {
        stderr.writeln('[VikunjaApiService] Raw Task created successfully with ID: ${createdVikunjaTask.id}');
      }

      final String serverId = targetServerOverride?.id ?? "vikunja_default";
      final createdTaskItem = _toTaskItem(createdVikunjaTask, serverId);
  ...
  Future<TaskItem> updateTask(
    ...
    try {
      ...
      final String serverId = targetServerOverride?.id ?? "vikunja_default";
      final updatedTaskItem = _toTaskItem(updatedVikunjaTask, serverId);
  ...
  TaskItem _toTaskItem(vikunja.ModelsTask vTask, String serverId) {
    // Use the factory constructor defined in TaskItem
    return TaskItem.fromVikunjaTask(vTask, serverId);
  }
  ...
  void _handleApiError(String context, dynamic error) {
    ...
    if (error is vikunja.ApiException) {
      ...
      errorMessage = error.message ?? 'Unknown API Exception (Code: $statusCode)';
      // Vikunja might return HTML for errors sometimes, try to extract message
      if (error.message != null && error.message!.contains('<title>')) {
         try {
            // Basic extraction, might need refinement
            final titleMatch = RegExp(r'<title>(.*?)<\/title>').firstMatch(error.message!);
            final bodyMatch = RegExp(r'<body>(.*?)<\/body>', dotAll: true).firstMatch(error.message!);
            if (titleMatch != null) errorMessage = titleMatch.group(1) ?? errorMessage;
            if (bodyMatch != null) errorMessage += "\n${bodyMatch.group(1)?.replaceAll(RegExp(r'<[^>]*>'), ' ').trim()}"; // Crude HTML strip
         } catch (_) {} // Ignore parsing errors
      }

      stderr.writeln('[VikunjaApiService] API Error - $context: $errorMessage');
