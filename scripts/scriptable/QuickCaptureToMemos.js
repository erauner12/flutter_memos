        ...
        if (processWithAi && config.openaiApiKey && originalInputText.trim().length >= MIN_LENGTH_FOR_AI) {
            ...
            try {
                ...
                if (confirmData && confirmData.useProcessed === true) {
                    ...
                    finalText = originalInputText;
                    console.log("User chose to revert to original text or cancelled confirmation (via WebView).");
                }
            ...
            } else if (config.openaiApiKey && originalInputText.trim().length < MIN_LENGTH_FOR_AI) {
                 console.log(`Text length (${originalInputText.trim().length}) is less than minimum (${MIN_LENGTH_FOR_AI}). Skipping AI processing.`);
            }
            finalText = originalInputText;
        }
