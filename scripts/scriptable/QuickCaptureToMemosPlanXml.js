...
async function presentWebViewForm(htmlContent, fullscreen = false) {
    // (Keep the existing presentWebViewForm function content - it's robust)
    // ... (same logic as previous version) ...
    console.log("Configuring interactive WebView form...");
    const wv = new WebView();
    wv.isPresented = false;
    ...
    try {
        ...
        while (true) {
            ...
            const evaluatePromise = wv.evaluateJavaScript(listenerScript, true);
                    ...
                    case "dictate":
                        ...
                        try {
                            ...
                            try {
                                ...
                                console.error("Failed to show dictation error alert in WebView:", alertError);
...
function generateConfigFormHtml(existingUrl, existingToken, existingOpenAIKey) {
    ...
    const openaiKeyPlaceholder = existingOpenAIKey ? `placeholder="Exists (Enter new to change)"` : `placeholder="Enter OpenAI Key"`; // Changed placeholder
    ...
    <body>
        ...
        <p>Enter your Memos instance URL, Access Token (OpenAPI), and your OpenAI API Key.</p>
        ...
        <form id="configForm">
            ...
            <label for="openaiKey">OpenAI API Key:</label> {/* Removed (Optional) */}
            <input type="password" id="openaiKey" name="openaiKey" ${openaiKeyPlaceholder}>
            <div id="openaiError" class="error" style="display: none;"></div> {/* Added error div */}
        ...
        function initializeForm() {
            try {
                ...
                const openaiError = document.getElementById('openaiError'); // Get error div

                if (!form || !urlInput || !tokenInput || !openaiInput || !urlError || !tokenError || !openaiError) {
                ...
                form.addEventListener('submit', (event) => {
                    ...
                    openaiError.style.display = 'none'; // Hide openai error
                    let isValid = true;
                    ...
                    // We'll let the main script validate if *some* token/key exists after submission
                    // No validation needed here for token/openai key presence
...
function generateInputFormHtml(prefillText = "") {
  const css = `
        ...
        .options { margin-bottom: 15px; display: flex; align-items: center; } /* Re-added options CSS */
        label[for=useAi] { margin-left: 8px; font-weight: normal; } /* Re-added label CSS */
        input[type=checkbox] { width: 18px; height: 18px; } /* Re-added checkbox CSS */
        h2 { margin-top: 0; color: #111; }
  ...
  const shareSheetNotice = prefillText
    ...
    : "";
  // AI Checkbox HTML is now ALWAYS included
  ...
  return `
    ...
    <body>
        ...
        <form id="inputForm">
            ...
            ${aiCheckboxHtml} <!-- Checkbox is always here -->
        ...
        <script>
            ...
            function initializeForm() {
                try {
                    ...
                    const contentInput = document.getElementById('memoContent');
                    const useAiCheckbox = document.getElementById('useAi'); // Checkbox should always exist now
                    const pasteButton = document.getElementById('pasteButton');
                    const dictateButton = document.getElementById('dictateButton');

                    // Check all elements including the checkbox
                    if (!form || !contentInput || !useAiCheckbox || !pasteButton || !dictateButton) {
                    ...
                    form.addEventListener('submit', (event) => {
                        ...
                        const processWithAi = useAiCheckbox.checked; // Get state directly
                        ...
                        if (content) {
                             if (typeof completion === 'function') {
                                completion({
                                    ...
                                    data: {
                                        text: content,
                                        useAi: processWithAi // Always include the checkbox state
                                    }
                                });
                            } else {
                    ...
                    dictateButton.addEventListener('click', () => {
                     ...
                     contentInput.focus();
                     console.log("Input form initialized with Paste/Dictate/AI Checkbox listeners and focused.");
        ...
        parsedPlanData.files.forEach((file) => {
            ...
            if (file.changes?.length > 0) {
                file.changes.forEach((change) => {
                    ...
                    if (change.search && (file.action === "delegate edit" || file.action === "modify")) {
                        changesHtml += `<div><strong>Search/Context:</strong><pre class="code-block search">${escapeHtml(change.search)}</pre></div>`;
                    }
                    if (change.content && file.action !== "delete") {
                        changesHtml += `<div><strong>Content/Change:</strong><pre class="code-block content">${escapeHtml(change.content)}</pre></div>`;
                    }
            ...
            <div class="file-block">
              <div class="file-header">
                <span class="file-action">${escapeHtml(file.action || "unknown")}</span>
                <span class="file-path">${escapeHtml(file.path || "No path specified")}</span>
              </div>
              ${changesHtml}
            </div>`;
        });
            ...
            function initializeForm() {
                try {
                    ...
                    if (!usePlanButton || !useOriginalButton) {
                         console.error("Plan review form elements not found.");
                         alert("Error initializing plan review form elements.");
                         if (typeof completion === 'function') completion({ error: "Initialization failed: Elements missing" });
                         return;
                    }
                    usePlanButton.addEventListener('click', () => {
                         if (typeof completion === 'function') { completion({ action: 'submit', data: { confirmedPlan: true } }); }
                         else { console.error('CRITICAL: completion function unavailable!'); alert('Error submitting choice.'); }
                    });
                    useOriginalButton.addEventListener('click', () => {
                         if (typeof completion === 'function') { completion({ action: 'submit', data: { confirmedPlan: false } }); }
                         else { console.error('CRITICAL: completion function unavailable!'); alert('Error submitting choice.'); }
                    });
...
async function getConfig(forcePrompt = false) {
    ...
    if (url && !url.toLowerCase().startsWith("http")) {
        ...
        Keychain.remove(KEYCHAIN_URL_KEY); url = null;
    }
    ...
    // --- Prompting Logic ---
    // Prompt ONLY if Memos URL/Token missing OR forced
    if (forcePrompt || !url || !token) {
        ...
        const newOpenaiApiKey = result.openaiApiKey;
        ...
        // Update OpenAI Key logic (same as before, handles saving/removing/keeping)
        if (newOpenaiApiKey) {
            openaiApiKey = newOpenaiApiKey;
            console.log(`Attempting to save NEW OpenAI Key to Keychain (length: ${openaiApiKey.length})...`);
            Keychain.set(KEYCHAIN_OPENAI_KEY, openaiApiKey);
            console.log("Saved new OpenAI API Key.");
        } else if (openaiApiKey && !newOpenaiApiKey) {
            console.log("OpenAI Key field left blank, removing existing key from Keychain.");
            openaiApiKey = null;
            if (Keychain.contains(KEYCHAIN_OPENAI_KEY)) Keychain.remove(KEYCHAIN_OPENAI_KEY);
        } else if (!openaiApiKey && !newOpenaiApiKey) {
            ...
            console.log("OpenAI Key field left blank, keeping existing key (no change to Keychain).");
        }
...
async function getInputText() {
    ...
    // Call generateInputFormHtml without the second argument (it always includes checkbox now)
    const inputHtml = generateInputFormHtml(initialText);
    const formData = await presentWebViewForm(inputHtml, false);

    // formData should now contain { text: "...", useAi: boolean } on success
    ...
    return { text: formData.text.trim(), useAi: formData.useAi }; // Return object
}
...
async function makeApiRequest(url, method, headers, body = null, timeout = 60, serviceName = "API") {
    ...
    try {
        ...
        if (method.toUpperCase() === 'GET' || expectJson) {
            try {
                ...
                responseText = JSON.stringify(responseData);
                 ...
                 console.warn(`${serviceName} loadJSON failed (Status: ${req.response?.statusCode}), trying loadString. Error: ${e.message}`);
                 statusCode = req.response?.statusCode ?? 500;
                 if (statusCode >= 200 && statusCode < 300) {
                     responseText = await req.loadString();
                     responseData = responseText;
                     console.log(`${serviceName} received non-JSON success response (Status: ${statusCode}).`);
                 } else {
                     responseText = await req.loadString().catch(() => "");
                     responseData = responseText;
                     console.error(`${serviceName} loadJSON failed and status code ${statusCode} indicates error.`);
                 }
            }
        } else {
          responseText = await req.loadString();
        ...
        if (e.message.startsWith(`${serviceName} Error`)) { throw e; }
...
async function getAiPlanAsXml(apiKey, userRequest) {
    // (Keep the existing getAiPlanAsXml function content - it's correct)
...
`;
    ...
    const messages = [ { role: "system", content: `You are a helpful assistant...${xmlFormattingInstructions}` }, { role: "user", content: `Generate a plan...${userRequest}` } ];
    const headers = { "Content-Type": "application/json", Authorization: `Bearer ${apiKey}` };
    const body = { model: model, messages: messages, max_tokens: 3500, temperature: 0.3, n: 1, stop: null };
    ...
    try {
        ...
        console.log("OpenAI XML plan generation successful. Raw XML length:", xmlContent.length);
...
function parseAiXmlResponse(xmlString) {
    ...
    try {
        ...
        parser.didStartElement = (name, attrs) => { /* ... */ }; // Keep existing logic
        parser.foundCharacters = (chars) => { /* ... */ }; // Keep existing logic
        parser.didEndElement = (name) => { /* ... */ }; // Keep existing logic
        parser.parseErrorOccurred = (line, column, message) => { /* ... */ }; // Keep existing logic

        // --- Re-paste the detailed parser logic from the previous version here ---
        parser.didStartElement = (name, attrs) => {
          currentTag = name.toLowerCase();
          accumulatedChars = "";
          if (currentTag === "file") {
            currentFile = { path: attrs.path || "", action: attrs.action || "", changes: [] };
          } else if (currentTag === "change") {
            currentChange = { description: "", content: "", complexity: null, search: "" };
          }
        };
        ...
        parser.didEndElement = (name) => {
          ...
          if (tagName === "chatname") { parsedData.chatName = trimmedChars; }
          else if (tagName === "plan") { parsedData.planText = trimmedChars; }
          else if (tagName === "file") {
            if (currentFile) {
              if (currentFile.path && currentFile.action) { parsedData.files.push(currentFile); }
              else { console.warn("Skipping file element missing path or action:", currentFile); }
              currentFile = null;
            }
          } else if (tagName === "change") {
            if (currentChange && currentFile) { currentFile.changes.push(currentChange); currentChange = null; }
          } else if (currentChange) {
            if (tagName === "description") { currentChange.description = trimmedChars; }
            else if (tagName === "content") { currentChange.content = trimmedChars.replace(/^===\s*|\s*===$/g, "").trim(); }
            else if (tagName === "complexity") { const v = parseInt(trimmedChars, 10); currentChange.complexity = isNaN(v) ? null : v; }
            else if (tagName === "search") { currentChange.search = trimmedChars.replace(/^===\s*|\s*===$/g, "").trim(); }
          }
...
function formatPlanForMemo(parsedPlanData) {
    ...
    let output = "";
    if (parsedPlanData.chatName) { output += `# ${parsedPlanData.chatName}\n\n`; }
...
(async () => {
  ...
  let config;
  let inputData; // Will hold { text: string, useAi: boolean }
  let createdMemo;
  let memoId;
  let finalText;
  let planUsed = false;
  ...
  try {
    // Get initial config (prompts only if Memos URL/Token missing or forced)
    config = await getConfig(forceConfigPrompt);
    console.log("Initial configuration obtained.");

    // Get user input (AI checkbox always shown)
    inputData = await getInputText();
    ...
    if (inputData.useAi) {
        ...
        // If key is missing, prompt the user to configure
        if (!hasValidOpenAIKey) {
            ...
            if (choice === 0) { // Configure Now
                ...
                try {
                    // Force the config prompt again and update config
                    config = await getConfig(true); // Pass true to force prompt
                    // Re-check if the key is valid *after* the prompt
                    ...
                    if (!hasValidOpenAIKey) {
                        ...
                        // Alert user that key is still missing?
                        ...
                        await stillMissingAlert.presentAlert();
                         ...
                         console.log("OpenAI Key configured successfully.");
                    }
                } catch (configError) {
                    console.error("Error during forced configuration:", configError);
                    // Treat config error/cancellation as reverting to original text
            ...
            } else { // Use Original Text (cancelled alert)
                console.log("User cancelled configuration prompt. Using original text.");
                hasValidOpenAIKey = false; // Ensure AI doesn't run
            }
        }
        ...
        if (hasValidOpenAIKey) {
            ...
            try {
                ...
                if (!parsedPlan) { throw new Error("Failed to parse the AI-generated XML plan."); }
