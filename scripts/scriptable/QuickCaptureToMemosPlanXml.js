...
function generateInputFormHtml(prefillText = "") {
    const css = `
        ...
        .ai-toggle { display: flex; align-items: center; } /* For the main AI toggle */
    ...
    // Generate checkboxes for common tasks
    let commonOptionsHtml = '<label>Common Tasks:</label><div class="common-options">';
    COMMON_AI_TASKS.forEach(task => {
        const id = `common-opt-${task.value}`;
        commonOptionsHtml += `
            <div>
                <input type="checkbox" id="${id}" name="commonOptions" value="${task.value}">
                <label for="${id}">${escapeHtml(task.label)}</label>
            </div>
        `;
    });
    commonOptionsHtml += '</div>';

    // Main AI toggle checkbox (checked by default)
    const aiToggleHtml = `
        <div class="ai-toggle">
    ...
    <form id="inputForm">
        ...
        <textarea id="composeInstructions" name="composeInstructions" placeholder="e.g., Summarize, make this more formal... (Combines with common tasks)"></textarea>
...
function generatePlanReviewHtml(parsedPlanData, originalContent = "") { // Added originalContent param
    ...
    // Section to display original input text
    const originalContentHtml = originalContent ? `
        <div class="original-content-section">
            <details>
                <summary>Show Original Input</summary>
                <div class="original-content-text">${escapeHtml(originalContent)}</div>
            </details>
        </div>
    ` : "";

    let proposalsHtml = "";
    if (parsedPlanData?.proposals?.length > 0) { /* ... (keep existing proposal rendering logic) ... */
        parsedPlanData.proposals.forEach((proposal, index) => { const proposalId = proposal.id || `prop-${index}`; const isChecked = proposal.action !== 'delete'; proposalsHtml += ` <div class="proposal-block"> <div class="proposal-header"> <span class="proposal-title">Proposal ${index + 1} (${escapeHtml(proposal.action || 'suggest')})</span> <div class="proposal-include"> <input type="checkbox" class="proposal-toggle" id="${proposalId}" name="${proposalId}" data-proposal-id="${proposalId}" ${isChecked ? 'checked' : ''}> <label for="${proposalId}">Include</label> </div> </div> <div class="proposal-content"> `; if (proposal.description) { proposalsHtml += `<div class="proposal-description">${escapeHtml(proposal.description)}</div>`; } if (proposal.original && ['replace', 'delete', 'keep'].includes(proposal.action)) { proposalsHtml += `<div class="text-block"><span class="text-label">Original${proposal.action === 'delete' ? ' (Deleted)' : (proposal.action === 'keep' ? ' (Kept)' : '')}:</span><div class="text-content original">${escapeHtml(proposal.original)}</div></div>`; } if (proposal.content && ['add', 'replace', 'keep'].includes(proposal.action)) { const label = proposal.action === 'replace' ? 'Proposed:' : (proposal.action === 'keep' ? 'Kept:' : 'Added:'); const contentClass = proposal.action === 'keep' ? 'text-content kept' : 'text-content added'; proposalsHtml += `<div class="text-block"><span class="text-label">${label}</span><div class="${contentClass}">${escapeHtml(proposal.content)}</div></div>`; } else if (proposal.action === 'add' && proposal.content) { proposalsHtml += `<div class="text-block"><span class="text-label">Added:</span><div class="text-content added">${escapeHtml(proposal.content)}</div></div>`; } proposalsHtml += `</div></div>`; });
    ...
    ${originalContentHtml} {/* Added original content display */}
...
async function getInputText() { /* ... (updated to return { text, compose, useAi, commonOptions }) ... */
    console.log("Checking for input source..."); let initialText = ""; if (args.plainTexts?.length > 0) { const sharedText = args.plainTexts.join("\n").trim(); if (sharedText) { console.log("Using text from Share Sheet."); initialText = sharedText; } } else { console.log("No Share Sheet input found."); } console.log(`Presenting WebView form for text input (AI checkbox always shown).`); const inputHtml = generateInputFormHtml(initialText); const formData = await presentWebViewForm(inputHtml, false);
    // Check for all expected fields now
    if (!formData || typeof formData.text === "undefined" || typeof formData.compose === "undefined" || typeof formData.useAi === "undefined" || !Array.isArray(formData.commonOptions)) {
        console.log("Input cancelled or form did not return expected data structure."); return null;
    }
    if (formData.text.trim() === "" && formData.compose.trim() === "" && formData.commonOptions.length === 0) {
        console.log("No text, instructions, or common options provided."); return null;
    }
    console.log(`Input received. Text length: ${formData.text.trim().length}, Compose length: ${formData.compose.trim().length}, Use AI: ${formData.useAi}, Common Options: [${formData.commonOptions.join(', ')}]`);
    return { text: formData.text.trim(), compose: formData.compose.trim(), useAi: formData.useAi, commonOptions: formData.commonOptions };
}
...
async function getAiPlanAsXml(apiKey, originalContent, userInstructions, commonOptions = []) { // Added commonOptions
    console.log("Requesting XML plan (proposal format) from OpenAI...");
    const endpoint = "https://api.openai.com/v1/chat/completions";
    const model = "gpt-4o";

    // Proposal XML Instructions (keep existing)
...
[...]`; // Truncated
    ...
    const hasCustomInstructions = userInstructions && userInstructions.trim().length > 0;
    const hasCommonOptions = commonOptions && commonOptions.length > 0;

    if (hasCommonOptions) {
        // Find labels for the selected values
        const selectedLabels = commonOptions
            .map(value => COMMON_AI_TASKS.find(task => task.value === value)?.label)
            .filter(label => label); // Filter out undefined if value not found
        if (selectedLabels.length > 0) {
            combinedInstructions += `Apply these actions: ${selectedLabels.join(', ')}. `;
        }
    }

    if (hasCustomInstructions) {
        if (combinedInstructions.length > 0) {
            combinedInstructions += `\nAdditionally: ${userInstructions}`;
        } else {
    ...
    let behaviorRule = "";
    if (originalContent && combinedInstructions) {
        behaviorRule = "Modify the 'Original Content' according to the 'User Instructions'.";
    } else if (combinedInstructions) {
        behaviorRule = "Generate new content based *only* on the 'User Instructions'.";
    } else if (originalContent) {
        behaviorRule = "Apply default improvements (fix grammar, spelling, clarity) to the 'Original Content'.";
        // Add default instruction if none provided
        combinedInstructions = "(Default: Fix grammar, spelling, and improve clarity of the Original Content.)";
    } else {
        // Neither content nor instructions provided - ask for example
        behaviorRule = "Generate a short example note as no content or instructions were given.";
        combinedInstructions = "Create a short example note.";
    }

    // --- Construct User Prompt ---
    let userPrompt = `Generate proposals in the specified XML format. ${behaviorRule}\n\n`;
    if (originalContent) {
        userPrompt += `**Original Content:**\n${originalContent}\n\n`;
    }
    // Only include instructions if they exist (custom or default)
...
(async () => {
  ...
  try {
    ...
    const commonOptions = inputData.commonOptions; // Get common options
    ...
    if (inputData.useAi) {
        ...
        if (hasValidOpenAIKey) {
            ...
            try {
                ...
                // Pass content, instructions, and common options
                ...
                // Pass original content to review screen
