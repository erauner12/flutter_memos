...
class ProjectObject {
  /// Returns a new [ProjectObject] instance.
  ProjectObject({
    required this.id,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is ProjectObject &&
    other.id == id &&
  ...
  @override
  String toString() => 'ProjectObject[id=$id, canAssignTasks=$canAssignTasks, childOrder=$childOrder, color=$color, createdAt=$createdAt, isArchived=$isArchived, isDeleted=$isDeleted, isFavorite=$isFavorite, isFrozen=$isFrozen, name=$name, updatedAt=$updatedAt, viewStyle=$viewStyle, defaultOrder=$defaultOrder, description=$description, parentId=$parentId, inboxProject=$inboxProject, isCollapsed=$isCollapsed, isShared=$isShared, collaboratorRoleDefault=$collaboratorRoleDefault, folderId=$folderId, isInviteOnly=$isInviteOnly, isLinkSharingEnabled=$isLinkSharingEnabled, role=$role, status=$status, workspaceId=$workspaceId]';
  ...
  /// Returns a new [ProjectObject] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static ProjectObject? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "ProjectObject[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "ProjectObject[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<ProjectObject> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <ProjectObject>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = ProjectObject.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, ProjectObject> mapFromJson(dynamic json) {
    final map = <String, ProjectObject>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = ProjectObject.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of ProjectObject-objects as value to a dart map
  static Map<String, List<ProjectObject>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<ProjectObject>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = ProjectObject.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
