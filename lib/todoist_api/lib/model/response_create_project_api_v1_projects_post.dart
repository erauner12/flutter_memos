...
class ResponseCreateProjectApiV1ProjectsPost {
  /// Returns a new [ResponseCreateProjectApiV1ProjectsPost] instance.
  ResponseCreateProjectApiV1ProjectsPost({
    required this.id,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is ResponseCreateProjectApiV1ProjectsPost &&
    other.id == id &&
  ...
  @override
  String toString() => 'ResponseCreateProjectApiV1ProjectsPost[id=$id, canAssignTasks=$canAssignTasks, childOrder=$childOrder, color=$color, createdAt=$createdAt, isArchived=$isArchived, isDeleted=$isDeleted, isFavorite=$isFavorite, isFrozen=$isFrozen, name=$name, updatedAt=$updatedAt, viewStyle=$viewStyle, defaultOrder=$defaultOrder, description=$description, parentId=$parentId, inboxProject=$inboxProject, isCollapsed=$isCollapsed, isShared=$isShared, collaboratorRoleDefault=$collaboratorRoleDefault, folderId=$folderId, isInviteOnly=$isInviteOnly, isLinkSharingEnabled=$isLinkSharingEnabled, role=$role, status=$status, workspaceId=$workspaceId]';
  ...
  /// Returns a new [ResponseCreateProjectApiV1ProjectsPost] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static ResponseCreateProjectApiV1ProjectsPost? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "ResponseCreateProjectApiV1ProjectsPost[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "ResponseCreateProjectApiV1ProjectsPost[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<ResponseCreateProjectApiV1ProjectsPost> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <ResponseCreateProjectApiV1ProjectsPost>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = ResponseCreateProjectApiV1ProjectsPost.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, ResponseCreateProjectApiV1ProjectsPost> mapFromJson(dynamic json) {
    final map = <String, ResponseCreateProjectApiV1ProjectsPost>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = ResponseCreateProjectApiV1ProjectsPost.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of ResponseCreateProjectApiV1ProjectsPost-objects as value to a dart map
  static Map<String, List<ResponseCreateProjectApiV1ProjectsPost>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<ResponseCreateProjectApiV1ProjectsPost>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = ResponseCreateProjectApiV1ProjectsPost.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
