...
class NoteListItemState extends ConsumerState<NoteListItem> {
  ...
  void _addNoteToWorkbenchFromList(
    ...
    // Use unawaited for fire-and-forget, error handling is inside the provider
    unawaited(ref.read(workbenchProvider.notifier).addItem(reference));

    // Show confirmation SnackBar using the passed context directly
    // REMOVED incorrect Builder wrapping
    ScaffoldMessenger.of(scaffoldContext).showSnackBar(
      SnackBar(
        // Use Material SnackBar for now, customize later if needed
        content: Text(
          'Added "${reference.previewContent.substring(0, min(30, reference.previewContent.length))}..." to Workbench',
        ),
        duration: const Duration(seconds: 2),
        backgroundColor: CupertinoColors.systemGreen, // Example customization
      ),
    );
  }
