...
class NotesPublicList200ResponseInner {
  /// Returns a new [NotesPublicList200ResponseInner] instance.
  NotesPublicList200ResponseInner({
    required this.id,
    required this.type,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is NotesPublicList200ResponseInner &&
    other.id == id &&
    other.type == type &&
  ...
  /// Returns a new [NotesPublicList200ResponseInner] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static NotesPublicList200ResponseInner? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "NotesPublicList200ResponseInner[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "NotesPublicList200ResponseInner[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return NotesPublicList200ResponseInner(
  ...
  static List<NotesPublicList200ResponseInner> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <NotesPublicList200ResponseInner>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = NotesPublicList200ResponseInner.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, NotesPublicList200ResponseInner> mapFromJson(dynamic json) {
    final map = <String, NotesPublicList200ResponseInner>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = NotesPublicList200ResponseInner.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of NotesPublicList200ResponseInner-objects as value to a dart map
  static Map<String, List<NotesPublicList200ResponseInner>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<NotesPublicList200ResponseInner>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = NotesPublicList200ResponseInner.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
