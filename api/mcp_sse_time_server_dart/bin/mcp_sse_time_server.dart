...
Future<void> main(List<String> args) async {
  ...
  try {
    ...
    server.listen(
      (HttpRequest request) {
        ...
        stderr
            .writeln('[TimeServer] Request: ${request.method} ${request.uri}');
        sseManager.handleRequest(request).catchError((e, s) {
          ...
          try {
            ...
            // Note: Checking persistentConnection might not be reliable here after error
            ...
            // Ignore errors during error response sending (e.g., connection already closed)
            stderr.writeln(
                '[TimeServer] Could not send error response for ${request.uri}. Connection likely closed.');
          }
        });
      },
      onError: (e, s) {
    ...
    stderr.writeln(
        '[TimeServer] FATAL: Failed to bind server to port $port: $e\n$s');
    exit(1); // Exit if server cannot bind
  }
  ...
  Future<void> shutdown() async {
    ...
    try {
      ...
      stderr.writeln(
          "[TimeServer] Closing ${sseManager.activeSseTransports.length} active SSE transports...");
      await Future.wait(
